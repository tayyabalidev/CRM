...
12. Paste and run `migrations/0010_notes_trigger_fix.sql` (fixes note create/update trigger accessing missing invoice_id).
13. Paste and run `migrations/0011_screenshot_notes.sql` (project and task screenshot notes with images).
14. Paste and run `migrations/0012_task_kind.sql` (separate bugs from tasks; clients can report bugs).
15. Paste and run `migrations/0013_onboarding_profile.sql` (first-workspace setup; restores profile write policies).
16. Copy the project URL and anon key into `.env.local` (and Vercel env for deploy).
